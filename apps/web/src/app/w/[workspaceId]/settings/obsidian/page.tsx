...
export default function ObsidianSettingsPage({
  ...
  const startMigration = async () => {
    ...
    const toastId = toast.loading("Importing your Obsidian vault…");
    const form = new FormData();
    form.append("workspace_id", workspaceId);
    form.append("file", file);
    try {
      for await (const ev of sseUpload<ImportEvent>("/api/obsidian/import", form)) {
        if (ev.type === "progress") {
          ...
          toast.success(
            `Import complete — ${ev.pages} page${ev.pages === 1 ? "" : "s"} added from your vault.`,
            { id: toastId }
          );
        }
      }
  ...
  return (
    <div className="mx-auto w-full max-w-[720px] flex-1 px-6 py-12">
      <header className="mb-8">
        <h1 className="text-3xl font-bold tracking-tight">Migrate from Obsidian</h1>
        <p className="mt-1 max-w-lg text-sm text-muted-foreground">
          Export your Obsidian vault as a <code>.zip</code> (or zip the vault folder) and upload it
          here. Your folders become nested pages, Markdown becomes native blocks, and{" "}
          <code>[[wikilinks]]</code> and embedded images are rewired to their imported pages. Nothing
          leaves your machine except this one upload.
        </p>
      </header>
      ...
      {finished !== null ? (
        <div className="rounded-xl border bg-card p-8 text-center">
          ...
          <p className="mx-auto mt-1 max-w-sm text-sm text-muted-foreground">
            Your Obsidian notes are now in the sidebar.
          </p>
          <Button
            variant="outline"
            className="mt-4"
              ...
              >
                {file ? (
                  ...
                  <>
                    ...
                    <span>Choose your vault .zip</span>
                  </>
                )}
              </button>
              <input
