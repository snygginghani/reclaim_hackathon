...
## 9.1 — Migrate from Notion via one-shot, self-revoking OAuth
"Connect Notion" runs a server-side migration (the OAuth token is a secret and Notion's API isn't CORS-reachable from the browser): `routers/notion.py` mints a Fernet-signed `state` (user+workspace, 15-min TTL, so the callback needn't rely on the short-lived access cookie), exchanges the code, and stores the token encrypted in a transient `notion_connections` row. `notion/importer.py` walks the workspace in two passes — build the page/database/row tree (`notion/convert.py` maps full block coverage + property/value shapes; assets are re-hosted because Notion's signed URLs expire in ~1h) then rewrite internal page-links and relation cell values from Notion ids to Lore ids. On **success** the token is revoked (`/v1/oauth/revoke`) and the row deleted, so Lore keeps no ongoing access; on **failure** the connection is kept so a retry needn't reconnect. Progress streams over SSE (reusing the Ollama-pull bar pattern); imported pages are indexed via the existing `ingest_page`.

Two real bugs found while building it: (a) imported database rows/properties FK to their page, so `_add_page` must `flush()` before dependent rows are added (same reason `create_database` flushes); (b) pass-2 link rewriting edits `Document.blocks` in place, which SQLAlchemy doesn't track for JSONB — needs `flag_modified(doc, "blocks")` or the resolved links never persist. Both covered by `tests/test_notion_import.py` (fake Notion client, asserts nesting, db mapping, link/relation resolution, asset re-host, and revoke-on-success).

## 9.2 — Local asset pipeline (also unlocks native image upload)
Full-fidelity Notion images needed somewhere to land: added `storage.py` + `routers/assets.py` (`POST /api/assets`) and a `StaticFiles` mount at `/uploads` (the `upload_dir` setting existed but was never wired). The importer re-hosts Notion media through the same `save_bytes`; the route also gives the editor a real upload target.
